...
base_folder = '../Images/Bench_Images/3-25-22/';
%tuned crop parameters to center the images
crop1 = [1 1080 1 1440];
%open images
im1 = open_im(strcat(base_folder, 'led2-lp2-0deg.png'));
im2 = open_im(strcat(base_folder, 'led2-lp2-60deg.png'));
im3 = open_im(strcat(base_folder, 'led2-lp2-120deg.png'));
...
h2 = image_data_struct(crop(im2, crop1), pi/3);
...
if flag_plot_hologram
    %noLP hologram plot
    hfig = figure('Name', 'noLP Hologram Plot');
    pos = get(hfig,'position');
    %make plot window wider
    set(hfig,'position',pos.*[0.25 0.8 2.5 1.2]); 
    subplot(2, 2, 1);
    plot_im(h1, 'noLP Image 1: \theta = 0');
    subplot(2, 2, 2);
    plot_im(h2, 'noLP Image 2: \theta = 2\pi/3');
    subplot(2, 2, 3);
    plot_im(h3, 'noLP Image 3: \theta = 4\pi/3');
    subplot(2, 2, 4);
    plot_im(c_hol, "noLP abs(Complex Hologram)");
end
flag_gen_3dhol = true;
if flag_gen_3dhol
    figure('Name', 'Generating Movie Scans')
    z_vals = linspace(-300, 0, 50); %focus seems to be ~-8mm
    %noLP 3D hologram focus ~-8.56mm (frame 145)
    data_hol_3d = hologram3D(c_hol, z_vals, PARAMS);
    %convert to movie frames
    data_frames = hologram3D_to_frames(data_hol_3d, 'noLP Pinhole');
    close
end
